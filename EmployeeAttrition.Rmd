...
ggplot(mapping = aes(x = MonthlyIncome, y = logit_of_prob + 0.01, color = Gender, group = Gender), data = attrition_income_gender) +
  geom_point(aes(size = count)) +
  geom_smooth() +
  xlab("Rounded MonthlyIncome") +
  ylab("Logit of the Probability of Employee Attrition") +
...
ggplot(mapping = aes(x = MonthlyIncome, y = logit_of_prob + 0.01, color = EducationField, group = EducationField), data = attrition_income_field) +
  geom_point(aes(size = count)) +
  geom_smooth() +
  xlab("Rounded MonthlyIncome") +
  ylab("Logit of the Probability of Employee Attrition") +
...
In this model, we replaced JobSatisfaction with isSatisfied which is a 4 on the 5 point scale of JobSatisfaction. With this switch, our AIC became 1133.6 which is a little higher compared to the AIC in the previous model. Even though the AIC is a bit higher, the deviance analysis confirms that all the variables are significant and neccesary in the model. Even though this is a good model, there are no intereaction terms so the next model would have Age and YearsInCurrentRole as interaction variables to see if there is an interaction between the two and if there is a significance. 
...
In the final model that we created, we included an interaction between Age and YearsInCurrentRole. With this interaction term added, the AIC dropped down to 1110.8. This is the lowest AIC when comaparing to all the other models that were created. All of the variables in this model are significant. The analysis of the deviance table support the addiction of the interaction term between Age and YearsInCurrentRole. 
...
```{r}
x = anova(attrition_model, test = "Chisq")
kable(x)
```
